...
### Key Findings
- **α = 0.05 wins (revenue & debt):** Across 9,567 ventures (2,451 participants), accelerator participation lifts Δ log(revenue) by 0.49–0.51 (≈+64–67%, p < 0.001) and raises FU1 debt incidence by +2.3 pp (p = 0.02) after program-level clustering.
- **Directional signals to monitor:** Employment gains remain fragile (≈+7%, p ≈ 0.02 in OLS/IPW but +0.6%, p = 0.43 under strict matching), equity incidence edges up +2.1 pp (p ≈ 0.07), and subgroup wins concentrate in South Asia, Latin America & Caribbean, and male- or mixed-led teams while women-led and thin-region cohorts retain wide intervals.
- **Robustness & estimator choice:** Propensity-score matching under a strict 0.10σ caliper (no replacement) keeps 27.7% of treated ventures and yields a muted +22% revenue ATT (p = 0.43) alongside the null employment result; relaxing to 0.20σ with replacement restores all treated ventures and raises the revenue ATT to +29% (p = 0.027), and a 0.10σ kernel estimator lands at +27% (p = 0.007). We cite those wins for completeness but foreground the strict-caliper benchmark when judging causal strength.
- **Program & gender dynamics:** Men-only teams post the largest revenue premium (+81%, p < 0.001) and mixed teams follow closely (+73%, p = 0.002); investor guarantees (+6.0 pp, p = 0.015) and sector focus (+9.7 pp, p < 0.001) correlate with higher FU1 equity conversion, whereas demo-day intensity remains negative and insignificant (−27%, p = 0.26), signalling these levers proxy for programme maturity rather than acting as clear causal switches.
...
```
    ...
    Time:                        16:32:40   Log-Likelihood:                -26469.
    ...
    Time:                        16:32:40   Log-Likelihood:                -11019.
...
**Section 1.5 Results:** The baseline PSM (caliper 0.10σ, no replacement) matches 678 of 2,451 treated ventures (27.7%) and delivers a much smaller, imprecise revenue effect (log 0.199 ≈ +22%, p = 0.43) alongside a null employment effect. Relaxing the caliper to 0.20σ with replacement brings coverage back to 100% and lifts the revenue ATT to ≈29% (p = 0.027), while a kernel estimator with 0.10σ bandwidth produces a +27% effect (p = 0.007) with similar overlap diagnostics.

**Narrative connection:** Matching highlights how fragile the revenue estimate becomes once we enforce close overlap. While OLS/IPW remain large and precise, the strict-caliper ATT loses significance—only the most comparable treated ventures sustain the accelerator premium. We therefore emphasise the conservative 0.10σ benchmark in the main text, cite the looser and kernel wins as supportive-but-less-comparable evidence, and keep the robustness narrative focused on why overlap, not estimator choice, drives the tension.
...
```
    ...
    Time:                        16:32:41   Log-Likelihood:                -26519.
...
```
    ...
    Time:                        16:32:41   Log-Likelihood:                -26539.
...
```
    ...
    Time:                        16:32:41   Log-Likelihood:                -25747.
...
<table border="1" class="dataframe">
  ...
  <tbody>
    <tr>
      <th>0</th>
      <td>H1: Revenue growth</td>
      <td>OLS w/ FE + clustered SEs</td>
      <td>63.8%</td>
      <td>17.1%</td>
      <td>0.000</td>
      <td>Supported</td>
      <td>Strict PSM (0.10σ) = 22.1% (p = 0.43)</td>
    </tr>
    <tr>
      <th>1</th>
      <td>H2: Employment growth</td>
      <td>OLS w/ FE + clustered SEs</td>
      <td>7.6%</td>
      <td>2.4%</td>
      <td>0.001</td>
      <td>Directional</td>
      <td>Strict PSM (0.10σ) = 8.1% (p = 0.05) — treat h...</td>
    </tr>
    <tr>
      <th>2</th>
      <td>H3: Equity access</td>
      <td>IPW linear probability</td>
      <td>2.1 pp</td>
      <td>1.1 pp</td>
      <td>0.057</td>
      <td>Directional</td>
      <td>Debt +2.3 pp (p = 0.02); equity lift remains m...</td>
    </tr>
    <tr>
      <th>3</th>
      <td>H4: Programme levers</td>
      <td>Participant-only equity GLM (clustered)</td>
      <td>Sector focus OR = 1.89</td>
      <td>—</td>
      <td>0.000</td>
      <td>Directional</td>
      <td>Investor guarantees OR = 1.53 (p = 0.015); dem...</td>
    </tr>
  </tbody>
</table>
</div>
...
<table border="1" class="dataframe">
  ...
  <tbody>
    <tr>
      <th>0</th>
      <td>H1a: Regional revenue uplift</td>
      <td>Latin America &amp; Caribbean, South Asia</td>
      <td>F</td>
      <td>23.99</td>
      <td>2</td>
      <td>393.0</td>
    ...
    <tr>
      <th>1</th>
      <td>H1b: Sector revenue uplift</td>
    ...
    <tr>
      <th>2</th>
      <td>H1c: Team-gender revenue uplift</td>
      <td>Men-Only, Mixed</td>
      <td>F</td>
      <td>12.33</td>
      <td>2</td>
      <td>393.0</td>
