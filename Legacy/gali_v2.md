...
### Key Findings
1. **Core Treatment Effect:** After conditioning on baseline performance, geography, and sector, participants exhibit an estimated **≈66%** revenue lift (log effect 0.51; 95% CI: 30%–104%) and a modest **≈7%** employment lift (log effect 0.07; 95% CI: 3%–12%) relative to comparable non-participants. Propensity-score matching with a tight caliper (0.10σ) only retains 27.7% of treated ventures and estimates a much smaller revenue lift (**≈22%**) with a wide, zero-crossing interval; expanding the caliper to 0.20σ (with replacement) covers 100% of treated ventures and raises the point estimate to ≈29%, but we treat that design as sensitivity because it relaxes the overlap requirements.
2. **Heterogeneity:** Revenue gains concentrate in **South Asia**, **Latin America & Caribbean**, and **North America**. Newly disaggregated Europe/MENA/East Asia buckets carry small sample sizes whose 95% confidence intervals cross zero; regional inferences outside the core geographies should therefore be treated as directional.
3. **Program & Gender Dynamics:** Men-only teams hold the largest estimated accelerator premium (+81%, p<0.001), mixed teams follow closely (+73%, p=0.002), and women-only teams register a positive but imprecise +26% lift amid smaller baseline scale. Investor guarantees (+6.0 pp) and sector focus (+9.7 pp) correlate with higher FU1 equity conversion, whereas demo-day intensity keeps a negative revenue coefficient (−27%, ns).
4. **Capital Pathways:** Conditioning on baseline financing, accelerator participation is associated with a **+2.1 pp** change in equity incidence (SE 1.1 pp; 95% CI: −0.1 to +4.3 pp) and a significant **+2.3 pp** rise in debt usage (p=0.02). A supplementary multinomial logit on joint equity/debt switches yields the same qualitative pattern but wide intervals, underscoring that equity effects are only marginally significant at the 10% level.
...
### Recommendations
1. **For accelerators:** Pair investor-facing features with disciplined operating support; monitor demo-day emphasis to avoid diluting revenue traction.
2. **For policymakers:** Focus ecosystem investments where evidence suggests the largest marginal gains (South Asia, Latin America & Caribbean) while prioritising data collection that can shrink wide intervals in under-represented regions.
3. **For entrepreneurs:** Accelerator participation correlates with stronger revenue trajectories for men-only and mixed teams and shows directional gains for women-led ventures; complementary capital-readiness support could help convert these gains into statistically precise equity outcomes.

### Limitations
- **Selection bias:** Even with rich covariates, unobserved founder quality may still bias estimates; results should be read as *consistent with* causal effects under selection-on-observables.
- **Attrition:** FU1 response hovers around 40%; IPW checks mitigate but cannot eliminate non-response concerns.
...
We summarise core variable families, the representative columns used in the analysis, their average completion rates, and the analytical role they play.
...
variable_families = [
    {
        ...
        'Columns': ['log_revenue_m1', 'log_ft_employees_m1', 'years_since_founding'],
        'Retained': 'Yes',
        'Role': 'Core controls for venture size and age'
    },
    {
        'Family': 'Capability & mission proxies',
        'Columns': ['digital_score', 'impact_intensity', 'has_ip'],
        'Retained': 'Yes',
        'Role': 'Captures digital readiness and impact orientation'
    },
    {
        'Family': 'Programme design levers',
        'Columns': ['program_demo_day_yes', 'program_curric_struct_yes', 'program_ben_ginv_yes', 'program_sector_focus_yes'],
        'Retained': 'Yes',
        'Role': 'Explains heterogeneity across accelerator offerings'
    },
    {
        'Family': 'Baseline financing mix',
        'Columns': ['inv_hasequity', 'inv_hasdebt', 'inv_philan_m1', 'inv_outequity_m1', 'inv_totaldebt_m1'],
        'Retained': 'Yes',
        'Role': 'Pre-program financing levels used as controls'
    },
    {
        'Family': 'FU1 outcomes',
        'Columns': ['delta_log_revenue', 'delta_log_ft', 'fu1inv_hasequity', 'fu1inv_hasdebt', 'fu1_responded'],
        'Retained': 'Yes',
        'Role': 'Primary outcomes and attrition adjustments'
    },
    {
        'Family': 'Founder biographies (multi-founder fields)',
        'Columns': [
            'found_name1_education', 'found_name2_education', 'found_name3_education',
            'found_name1_prior_fp', 'found_name2_prior_fp', 'found_name3_prior_fp'
        ],
        'Retained': 'No',
        'Role': 'Dropped due to >80% missingness and inconsistent coverage'
    },
    {
        'Family': 'Later follow-ups (FU2–FU4)',
        'Columns': [c for c in df.columns if c.startswith('fu2') or c.startswith('fu3') or c.startswith('fu4')],
        'Retained': 'No',
        'Role': 'Excluded because completion falls below 20% beyond FU1'
    }
]

records = []
for fam in variable_families:
    cols = [c for c in fam['Columns'] if c in df.columns]
    if cols:
    ...
    records.append({
        ...
        'Retained': fam['Retained'],
        'Avg coverage %': round(coverage, 1) if coverage == coverage else float('nan'),
        'Analytical role / justification': fam['Role']
    })

variable_family_table = pd.DataFrame(records)
variable_family_table = variable_family_table.sort_values(['Retained', 'Avg coverage %'], ascending=[False, False])
...
<table border="1" class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th></th>
      <th>Family</th>
      <th>Representative columns</th>
      <th>Retained</th>
      <th>Avg coverage %</th>
      <th>Analytical role / justification</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      ...
      <td>inv_hasequity, inv_hasdebt, inv_philan_m1 …</td>
      <td>Yes</td>
      <td>100.0</td>
      <td>Pre-program financing levels used as controls</td>
    </tr>
    <tr>
      <th>2</th>
      <td>Programme design levers</td>
      <td>program_demo_day_yes, program_curric_struct_ye...</td>
      <td>Yes</td>
      <td>87.3</td>
      <td>Explains heterogeneity across accelerator offe...</td>
    </tr>
    <tr>
      <th>0</th>
      ...
      <td>Yes</td>
      <td>NaN</td>
      <td>Core controls for venture size and age</td>
    </tr>
    <tr>
      <th>1</th>
      ...
      <td>Yes</td>
      <td>NaN</td>
      <td>Captures digital readiness and impact orientation</td>
    </tr>
    <tr>
      <th>4</th>
      <td>FU1 outcomes</td>
      ...
      <td>Yes</td>
      <td>NaN</td>
      <td>Primary outcomes and attrition adjustments</td>
    </tr>
    <tr>
      <th>5</th>
      ...
      <td>No</td>
      <td>83.8</td>
      <td>Dropped due to &gt;80% missingness and inconsiste...</td>
    </tr>
    <tr>
      <th>6</th>
      <td>Later follow-ups (FU2–FU4)</td>
      <td>fu2report_followup_yes, fu2New_External_ID, fu...</td>
      <td>No</td>
      <td>11.6</td>
      <td>Excluded because completion falls below 20% be...</td>
    </tr>
  </tbody>
</table>
</div>
...
![png](gali_v2_files/gali_v2_12_1.png)
...
![png](gali_v2_files/gali_v2_17_0.png)
...
![png](gali_v2_files/gali_v2_21_1.png)
...
![png](gali_v2_files/gali_v2_31_0.png)
...
- **H1c (Team Gender Heterogeneity):** Men-only founding teams capture the largest acceleration premium, mixed teams follow closely, and women-only teams may see muted or imprecise lifts.
...
![png](gali_v2_files/gali_v2_47_2.png)
...
```
    ...
    Time:                        09:32:57   Log-Likelihood:                -26469.
    ...
    Time:                        09:32:57   Log-Likelihood:                -11019.
...
![png](gali_v2_files/gali_v2_66_0.png)
...
**Section 1.6 Results:** Balance diagnostics confirm that the strict-caliper match dramatically improves observable alignment but leaves only 27.7% of treated ventures in the matched sample, yielding a noisy revenue ATT (log 0.199 ≈ +22%) whose confidence interval straddles zero and an employment effect that remains null. Pushing the caliper to 0.20σ with replacement restores full coverage (100% of treated) and raises the revenue point estimate to ≈29%, yet Love plots show slightly looser post-match balance on a few programme indicators.

**Narrative connection:** The diagnostics make the trade-off transparent: enforcing stringent overlap reduces bias at the expense of power, while looser matching buys precision by allowing more distant controls. Because the qualitative story (positive but highly uncertain revenue lift, no employment effect) survives both specifications, we carry forward the conservative baseline caliper and relegated the broader match to sensitivity notes.
...
```
    ...
    Time:                        09:32:58   Log-Likelihood:                -26519.
...
![png](gali_v2_files/gali_v2_73_2.png)
...
```
    ...
    Time:                        09:32:58   Log-Likelihood:                -26539.
...
![png](gali_v2_files/gali_v2_78_2.png)
...
```
    ...
    Time:                        09:32:58   Log-Likelihood:                -25747.
...
![png](gali_v2_files/gali_v2_88_2.png)
...
![png](gali_v2_files/gali_v2_93_1.png)
...
![png](gali_v2_files/gali_v2_97_1.png)
...
![png](gali_v2_files/gali_v2_98_3.png)
...
![png](gali_v2_files/gali_v2_105_2.png)
...
![png](gali_v2_files/gali_v2_119_1.png)
...
![png](gali_v2_files/gali_v2_123_1.png)
...
![png](gali_v2_files/gali_v2_133_0.png)
...
![png](gali_v2_files/gali_v2_135_1.png)
...
![png](gali_v2_files/gali_v2_139_9.png)
...
prob_plot_df = analysis_df[['participated', 'response_prob']].dropna().copy()
prob_plot_df['Treatment Status'] = prob_plot_df['participated'].map({1: 'Participant', 0: 'Non-participant'})

sns.histplot(
    data=prob_plot_df,
    x='response_prob',
    hue='Treatment Status',
    stat='density',
    common_norm=False,
    binwidth=0.05,
    ax=axes[0]
)
axes[0].set_title('Predicted FU1 Response Probability by Treatment')
axes[0].set_xlabel('Predicted response probability')
axes[0].set_ylabel('Density')

ipw_plot_df = analysis_df[['response_prob', 'ipw', 'participated']].dropna().copy()
ipw_plot_df['Treatment Status'] = ipw_plot_df['participated'].map({1: 'Participant', 0: 'Non-participant'})
...
axes[1].set_title('IPW Leverage vs Predicted Response Probability')
axes[1].set_xlabel('Predicted response probability (clip=0.05)')
axes[1].set_ylabel('Inverse probability weight (log scale)')
axes[1].legend()

# Annotate the five highest weights for transparency
top_weights = ipw_plot_df.nlargest(5, 'ipw')
for _, row in top_weights.iterrows():
    axes[1].annotate(
        f"{row['Treatment Status'][0]}-{row['ipw']:.1f}",
        (row['response_prob'], row['ipw']),
...
![png](gali_v2_files/gali_v2_141_0.png)
...
![png](gali_v2_files/gali_v2_146_0.png)
...
![png](gali_v2_files/gali_v2_146_4.png)
...
![png](gali_v2_files/gali_v2_148_0.png)
...
![png](gali_v2_files/gali_v2_152_0.png)
    


    Influential observations (Cook's D > 0.0004): 429
      (4.53% of sample)
...
![png](gali_v2_files/gali_v2_160_0.png)
...
![png](gali_v2_files/gali_v2_173_0.png)
...
**Primary Research Question:** Are venture outcomes for accelerator participants consistent with acceleration-driven gains relative to comparable applicants?

**Answer:** The evidence points to sizeable revenue improvements and modest employment gains that are **consistent with** a positive acceleration effect under selection-on-observables, with important nuance across geography, sector, gender, and programme design.

1. **Main Treatment Effects (H1–H2):**
   - Fixed-effects OLS estimates a **66.8%** revenue lift (p<0.001) and a **7.2%** full-time employment gain (p<0.01) relative to comparable non-participants.
   ...
   - Attrition-adjusted WLS (IPW, clip=0.05) produces a **66.0%** revenue effect, reinforcing robustness to differential follow-up while acknowledging lingering attrition risk.

2. **Heterogeneity Insights (H1a & H1b/H1c):**
   - **South Asia** (+503%, p<0.001), **Latin America & Caribbean** (+80%, p=0.001), and **North America** (+57%, p=0.016) are the clearest regional winners. Sub-Saharan Africa’s +23% estimate is imprecise, and the newly disaggregated Europe/MENA/East Asia buckets have small Ns with 95% CIs that cross zero.
   - **Health** (+146%, p=0.005) and **Education** (+71%, p=0.042) ventures top the sector ranking, while Agriculture and Financial Services deliver moderate, noisy lifts that straddle zero.
   - Men-only (+81%, p<0.001) and mixed teams (+73%, p=0.002) capture statistically meaningful gains; women-only teams show a directional +26% effect, echoing the lower baseline revenue/digital footprint documented in Section 3.1.

3. **Mechanisms & Capital Pathways (H3 & H4):**
   - FU1 equity incidence is 19.7% for participants vs 16.8% for controls; the baseline-controlled LPM estimates a **+2.1 pp** equity change (SE 1.1 pp; 95% CI: −0.1 to +4.3 pp; p=0.07) and a significant **+2.3 pp** rise in debt incidence (p=0.02).
   - A multinomial logit on joint equity/debt switches points in the same direction but with wide intervals, reinforcing that equity effects are marginally significant at best.
   - Investor guarantees and sector focus correlate with higher equity conversion among participants, yet we treat these as descriptive associations pending richer controls for programme selection.

4. **Robustness & Diagnostics:** Matching diagnostics show all covariate standardised mean differences shrink below |0.1| post-match. Attrition modelling highlights meaningful variation in response probabilities, and IPW leverage plots identify a manageable set of high-weight observations. Baseline vs outcome plots show the log-linear specification fits most of the support, with a few high-revenue outliers that are handled by winsorization.

### 12.2 Implications
- **Policy/Practice:** Focus placement of accelerator funding where estimated gains are largest, but pair quantitative monitoring with qualitative assessments to understand why certain programmes outperform; invest in data systems to firm up under-represented regions.
- **Programme Design:** Treat demo days, investor guarantees, and sector focus as promising signals rather than guaranteed levers; complementary mentoring and capital-readiness support appear critical for women-led teams.
- **Future Research:** Richer founder-level data, better tracking of repeat applicants, and experimental/pseudo-experimental designs would help distinguish selection from causal impact.

### 12.3 Limitations
- Residual selection bias may remain due to unobserved founder quality or programme vetting.
- FU1 response rates (~41%) necessitate IPW and leave room for differential attrition bias.
- Funding-mix estimates rely on self-reported financing categories that may mask nuanced investment structures.

Overall, the notebook provides a transparent set of diagnostics and robustness checks that make the acceleration effects *plausible* under the maintained assumptions, while clearly documenting areas where stronger evidence is still required.
...
<table border="1" class="dataframe">
  <thead>
    <tr style="text-align: right;">
      ...
      <th>Description</th>
      <th>Raw source count</th>
      <th>Ventures with raw data (&gt;0)</th>
      <th>Ventures flagged = 1</th>
      <th>Ventures flagged = 0 but raw &gt; 0</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <th>0</th>
      <td>fu1inv_hasequity</td>
      <td>Flag if any FU1 equity source reported</td>
      <td>17</td>
      <td>1679</td>
      <td>1679</td>
      <td>0</td>
    ...
    <tr>
      <th>1</th>
      <td>fu1inv_hasdebt</td>
      <td>Flag if any FU1 debt source reported</td>
      <td>15</td>
      <td>1400</td>
      <td>1400</td>
      <td>0</td>
    ...
    <tr>
      <th>2</th>
      <td>fu1inv_hasphilan</td>
      <td>Flag if any FU1 philanthropy source reported</td>
      <td>10</td>
      <td>2922</td>
      <td>2922</td>
      <td>0</td>
...
<table border="1" class="dataframe">
  ...
  <tbody>
    <tr>
      <th>0</th>
      <td>Baseline log revenue</td>
      <td>9567</td>
      <td>0.0</td>
    </tr>
    <tr>
      <th>1</th>
      <td>Baseline log FTE</td>
      <td>9567</td>
      <td>0.0</td>
    </tr>
    <tr>
      <th>2</th>
      <td>FU1 revenue change</td>
      <td>9567</td>
      <td>0.0</td>
    </tr>
    <tr>
      <th>3</th>
      <td>FU1 FTE change</td>
      <td>9567</td>
      <td>0.0</td>
    </tr>
    <tr>
      <th>4</th>
      <td>FU1 equity incidence</td>
      <td>9567</td>
      <td>0.0</td>
    </tr>
    <tr>
      <th>5</th>
      <td>FU1 debt incidence</td>
      <td>9567</td>
      <td>0.0</td>
    </tr>
    <tr>
...
![png](gali_v2_files/gali_v2_181_1.png)
...
<table border="1" class="dataframe">
  ...
  <tbody>
    <tr>
      <th>0</th>
      <td>Baseline revenue (USD, exp(log) − 1)</td>
      <td>788287.78</td>
      <td>52640408.48</td>
      <td>9567</td>
    </tr>
    <tr>
      <th>1</th>
      <td>Baseline FT employees (exp(log) − 1)</td>
      <td>164.47</td>
      <td>15338.98</td>
      <td>9567</td>
    </tr>
    ...
    <tr>
      <th>3</th>
      <td>Equity incidence (baseline, %)</td>
      <td>15.24</td>
      <td>35.94</td>
      <td>9567</td>
    </tr>
...
<table border="1" class="dataframe">
  <thead>
    <tr style="text-align: right;">
      ...
      <th>p-value</th>
      <th>N (effective)</th>
      <th>CI lower (effect)</th>
      <th>CI upper (effect)</th>
      <th>Effect (pp)</th>
    </tr>
  </thead>
  <tbody>
...
![png](gali_v2_files/gali_v2_189_1.png)
